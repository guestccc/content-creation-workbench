...
| 图像处理 | numpy + Pillow | 一键换背景的抠图/合成：纯算术、无模型无网络，同输入结果逐字节相同 |
...
```
materials/
├── source/     ① 原始素材：待切的视频、待换背景的图片往这里拷（页面的默认输入目录）
...
| `dubbing/` | 智能配音产物（.wav 等音频） | 后端。智能配音页每次生成一份 `<名字>.wav`，重名追加 `-2`、`-3` |
| `background/` | 一键换背景产物（换好底的整图） | 后端。换背景页每次任务建 `background-<时间戳>/` 子目录 |
...
- 入库：`materials/README.md`（下面那份说明）与各分段的 `.gitkeep` 占位文件
...
删了、或者把 `SCENE_MATERIALS_DIR` 指到别处，下次启动都是齐齐的八段。
...
- `/api/v1/fs/list` 与 `/api/v1/fs/preview`（素材列表的预览流）是面向本机
  单用户的无鉴权接口，后者会真的把本地视频吐出去，
