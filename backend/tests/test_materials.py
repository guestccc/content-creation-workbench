...
1. 骨架是**八段齐全**的 —— 少建一段，新克隆的仓库就会退化成随便堆的一层；
...
#: 规划里的八个分段，写成字面量而不是从 SUBDIRS 取 —— 断言才有意义，
#: 否则拿 SUBDIRS 跟自己比，删掉一段测试照样绿。
EXPECTED_SUBDIRS = [
    "source", "clips", "subtitle", "output",
    "crawl", "finalcut", "dubbing", "background",
]


def test_plan_has_exactly_eight_segments():
    """规划就是按流程分的八段（视频四段 + 图文抓取 + 一键成品 + 智能配音 + 一键换背景），
    顺序也固定（文档与日志都按这个顺序写）。"""
    assert list(SUBDIRS) == EXPECTED_SUBDIRS
    assert {
        SOURCE, CLIPS, SUBTITLE, OUTPUT, CRAWL, FINALCUT, DUBBING, BACKGROUND
    } == set(EXPECTED_SUBDIRS)


def test_ensure_materials_layout_creates_all_subdirs(tmp_path, monkeypatch):
    """骨架建成：根目录 + 八个分段，每一段都是真目录。"""
