...
- 队列就是 crawl_jobs 表里 status='pending' 的行，这里没有第二份队列状态；
- 执行逻辑全在 CrawlRunner 里，本模块只是一个循环外壳；
- start/stop 只由 lifespan 调用 —— uvicorn --reload 的父进程会 import 模块
  但永不调用 lifespan，所以 worker 放模块级会起两份、抢同一个 SQLite。

单线程 = 同时只跑一个抓取任务，这是刻意保留的保护而不是缺陷：多个平台
同时抓会互相抢风控额度，单任务串行对账号更安全。
...
from app.services.crawl_runner import CrawlRunner, claim_next_pending_id
...
class CrawlJobWorker:
    ...
    def __init__(
        ...
        *,
        ...
        runner_factory: Callable[[], CrawlRunner] = CrawlRunner,
        ...
        self.poll_seconds = (
            poll_seconds if poll_seconds is not None else settings.CRAWL_JOB_POLL_SECONDS
        )
    ...
    def stop(self, grace: Optional[float] = None) -> None:
        ...
        if thread.is_alive():
            logger.warning("素材抓取工作线程未在宽限期内退出，交由 daemon 兜底")
        else:
            logger.info("素材抓取工作线程已停止")
        self._thread = None
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                logger.exception("素材抓取工作循环出现异常，继续运行")
                self._stop.wait(self.poll_seconds)


# 模块级单例：只在 app.main 的 lifespan 里 start/stop，别处不要动它
crawl_job_worker = CrawlJobWorker()
