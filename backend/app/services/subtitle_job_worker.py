...
与 scene_job_worker.py 完全同构，刻意保持薄：
- 队列就是 subtitle_jobs 表里 status='pending' 的行，这里没有第二份队列状态；
- 执行逻辑全在 SubtitleRunner 里，本模块只是一个循环外壳；
...
from app.services.subtitle_runner import SubtitleRunner, claim_next_pending_id

logger = get_logger(__name__)


class SubtitleJobWorker:
    ...
    def __init__(
        ...
        *,
        ...
        runner_factory: Callable[[], SubtitleRunner] = SubtitleRunner,
        ...
        self.poll_seconds = (
            poll_seconds if poll_seconds is not None else settings.SUBTITLE_JOB_POLL_SECONDS
        )
    ...
    def stop(self, grace: Optional[float] = None) -> None:
        ...
        if thread.is_alive():
            logger.warning("字幕提取工作线程未在宽限期内退出，交由 daemon 兜底")
        else:
            logger.info("字幕提取工作线程已停止")
        self._thread = None
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                logger.exception("字幕提取工作循环出现异常，继续运行")
                self._stop.wait(self.poll_seconds)


# 模块级单例：只在 app.main 的 lifespan 里 start/stop，别处不要动它
subtitle_job_worker = SubtitleJobWorker()
