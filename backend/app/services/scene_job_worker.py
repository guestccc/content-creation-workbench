...
全仓库唯一碰 threading 的地方（约几十行），刻意保持薄：
- 队列就是 scene_jobs 表里 status='pending' 的行，这里没有第二份队列状态；
- 执行逻辑全在 SceneRunner 里，本模块只是一个循环外壳；
- start/stop 只由 lifespan 调用 —— uvicorn --reload 的父进程会 import 模块
  但永不调用 lifespan，所以 worker 放模块级会起两份、抢同一个 SQLite。

测试说明：claim_next / executor 都可注入，唯一那个线程用例完全不碰 DB。
"""
...
from app.services.scene_runner import SceneRunner, claim_next_pending_id

logger = get_logger(__name__)


class SceneJobWorker:
    ...
    def __init__(
        ...
        *,
        ...
        runner_factory: Callable[[], SceneRunner] = SceneRunner,
        ...
        self.poll_seconds = (
            poll_seconds if poll_seconds is not None else settings.SCENE_JOB_POLL_SECONDS
        )
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._loop,
            name="scene-job-worker",
            # daemon：主进程退出时不等它 —— 但 lifespan shutdown 会先 stop() 有界等待，
            # daemon 只是双 Ctrl-C 这类硬退出下的兜底
            daemon=True,
        )
        self._thread.start()
        logger.info("镜头分割工作线程已启动 | 轮询间隔=%ss", self.poll_seconds)

    def stop(self, grace: Optional[float] = None) -> None:
        """停止工作线程：置位停止信号并有界等待线程退出。

        等待必须有界 —— uvicorn reloader 的 join() 没有超时，这里若无界，
        改一次代码热重载就要卡到当前视频切完（可能几分钟）。超时后线程
        仍是 daemon，会随主进程退出被强杀，残留子进程由下次启动的
        recover_interrupted_jobs() 回收。
        """
        self._stop.set()
        thread = self._thread
        if thread is None:
        ...
        if thread.is_alive():
            logger.warning("镜头分割工作线程未在宽限期内退出，交由 daemon 兜底")
        else:
            logger.info("镜头分割工作线程已停止")
        self._thread = None
    ...
    def _loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                logger.exception("镜头分割工作循环出现异常，继续运行")
                self._stop.wait(self.poll_seconds)


# 模块级单例：只在 app.main 的 lifespan 里 start/stop，别处不要动它
scene_job_worker = SceneJobWorker()
